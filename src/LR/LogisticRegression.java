...
public class LogisticRegression {
    ...
    private void XMatrix(){
        ...
        //copies data set with a 1 at first
        //it doesnt copy the last y value.
    ...
    private void YMatrix(){
        Y = new double[rows][1];
        //this gets the y result of each row:
        for (int i = 0; i < rows; i++){
        ...
        for (int i = 0; i < 8000; i++) {
            ...
            for (int j = 0; j < X.length; j++) {
               for (int k = 0; k < X[0].length; k++) {
                    sigma += ((1 / ( 1 + Math.exp( - Operations.sigmoid(k,Ws,X))))- Y[k][0] ) * X[j][k];
                }
